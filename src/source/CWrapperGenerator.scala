...
class CWrapperGenerator(spec: Spec) extends Generator(spec) {
  ...
  def generateInterface(origin: String, ident: Ident, doc: Doc, typeParams: Seq[TypeParam], i: Interface, deprecated: scala.Option[Deprecated]): Unit = {
  ...
  def generateEnum(origin: String, ident: Ident, doc: Doc, e: Enum, deprecated: scala.Option[Deprecated]): Unit = {
    val refs = new CRefs(ident, origin)
  ...
  def generateRecord(origin: String, ident: Ident, doc: Doc, params: Seq[TypeParam], r: Record, deprecated: scala.Option[Deprecated], idl: Seq[TypeDecl]): Unit = {
    ...
    superRecord match {
      case None => {}
      case Some(value) => {
        refs.hpp.add("#include " + q(marshal.dh + value.ident.name + ".hpp") + marshal.pythonCdefIgnore)
      }
    }
    ...
    writeCHeader(marshal.dh + ident.name, origin, "", refs.h, true, w => {
      ...
      declareGlobalSignaturesRecordFieldGetters(ident, r, "", 0, w, superFields)
    })
  ...
  def declareGlobalRecordFieldGetters(ident: Ident, r: Record, prefix: String, number: Int, w: IndentWriter, superFields: Seq[Field]) = {
    val recordAsMethodName = idCpp.method(ident.name)
  ...
  def declareGlobalSignaturesRecordFieldGetters(ident: Ident, r: Record, prefix: String, number: Int, w: IndentWriter, superFields: Seq[Field]) = {
    val recordAsMethodName = idCpp.method(ident.name)
    var field_number = number
    for (f <- (superFields ++ r.fields)) {
      field_number += 1
  ...
  def writeRecordToCpp(ident: Ident, r: Record, prefix: String, number: Int, w: IndentWriter, superFields: Seq[Field]) = {
  ...
  def writeRecordFromCpp(ident: Ident, r: Record, prefix: String, number: Int, w: IndentWriter, superFields: Seq[Field]) = {
    val recordAsMethodName = idCpp.method(ident.name)
