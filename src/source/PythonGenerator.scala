...
class PythonGenerator(spec: Spec) extends Generator(spec) {
  ...
  override def generateInterface(origin: String, ident: Ident, doc: Doc, typeParams: Seq[TypeParam], i: Interface, deprecated: scala.Option[Deprecated]): Unit = {
    ...
    writePythonFile(ident, origin, refs.python, true, w => {
      ...
      w.wl("class " + pythonClass + "(with_metaclass(ABCMeta)):").nested {
        ...
        if (writeDocString(w, doc, docConsts)) {
          w.wl
        }
  ...
  override def generateRecord(origin: String, ident: Ident, doc: Doc, params: Seq[TypeParam], r: Record, deprecated: scala.Option[Deprecated], idl: Seq[TypeDecl]): Unit = {
    ...
    // Handle super class import.
    superRecord match {
      case None =>
      case Some(value) => {
        refs.collect(value.ident, value.record)
      }
    }
    ...
    writePythonFile(ident.name + (if (r.ext.py) "_base" else ""), origin, refs.python, true, w => {
      ...
      w.wl("class " + recordClassName + (if (r.ext.py) "Base" else "") + (marshal.extendsRecord(idl, r)) + ":").nested {
        ...
        if (writeDocString(w, doc, docLists)) {
          w.wl
        }
        ...
        if (r.derivingTypes.nonEmpty) {
          ...
          writeRecordDerivings(r, w, superFields)
        }
        w.wl

        // Constructor
        w.wl("def __init__" + getRecordArguments(r, "self", superFields).mkString("(", ", ", ")") + ":").nested {

          // write super fields initialize
          if (superFields.nonEmpty) {
            val superFiledsStringName = superFields.map(f => idPython.field(f.ident.name))
            w.wl("super().__init__" + superFiledsStringName.mkString("(", ", ", ")"))
          }
  ...
  def getRecordArguments(r: Record, self: String, superFields: Seq[Field] = Seq.empty) = {
    val fields = (superFields ++ r.fields)
    if (self != "") Seq(self) ++ fields.map(f => idPython.field(f.ident.name))
  ...
  def writeRecordCallbacks(ident: Ident, r: Record, number: Int, callbackNames: mutable.Set[String], w: IndentWriter, superFields: Seq[Field] = Seq.empty) = {
    val recordAsMethod = idPython.method(ident.name)
  ...
  def writeRecordToFromCb(ident: Ident, r: Record, prefix: String, number: Int, w: IndentWriter, superFields: Seq[Field] = Seq.empty) = {
    val recordClassName = idPython.className(ident.name)
    val skipFirst = SkipFirst()
  ...
  def writeRecordDerivings(r: Record, w: IndentWriter, superFields: Seq[Field] = Seq.empty): Unit = {
    // EQUAL
    ...
    w.wl("def __hash__(self):").nested {
      ...
      w.wl("hash_code = 17")
      for (f <- (superFields ++ r.fields)) { // TODO: should we check that if f is a record it has derivings? we don't in Java
  ...
  class PythonRefs(ident: Ident, origin: String) {
    ...
    def collect(ty: TypeRef, justCollect: Boolean) {
      collect(ty.resolved, justCollect, false)
    }
    ...
    def collect(ident: Ident, record: Record) = for (r <- marshal.references(ident, record)) r match {
      case ImportRef(arg) => {
        python.add(arg)
      }
      case _ =>
